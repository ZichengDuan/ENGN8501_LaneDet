...
class _BezierLaneDataset(torchvision.datasets.VisionDataset):
    ...
    def __init__(self, root, image_set='train', transforms=None, transform=None, target_transform=None,
                 order=3, num_sample_points=100, aux_segmentation=False, have_aug=False, aug_rate=0.2):
        ...
        if image_set != 'valfast':
            ...
            if self.have_aug:
                self.auged_bezier_labels = os.path.join(self.bezier_labels_dir, 'train_augment' + '_' + str(order) + '.json')
    ...
    def loader_bezier(self):
        ...
        if self.have_aug:
            aug_results = []
            with open(self.auged_bezier_labels, 'r') as f:
                aug_results += [json.loads(x.strip()) for x in f.readlines()]
            for lanes in aug_results:
                temp_lane = []
                for lane in lanes['bezier_control_points']:
                    temp_cps = []
                    for i in range(0, len(lane), 2):
    ...
    def _init_all(self):
        ...
        # append aug
        if self.have_aug:
            aug_split_f = os.path.join(self.splits_dir, 'train_augment.txt')
            with open(aug_split_f, "r") as f:
                aug_contents = [x.strip() for x in f.readlines()]
            self.num_aug = int(self.aug_rate * len(aug_contents))
            self.first_aug_idx = len(contents)
            contents.extend(aug_contents)
