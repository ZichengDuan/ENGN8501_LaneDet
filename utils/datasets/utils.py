...
def pair_collate_fn(batch, extra_sampling):
    batch_idx = list(range(len(batch)))
    extract_idx = random.sample(batch_idx, extra_sampling)
    # img = [_[0] for _ in batch]
    # style_img = [_[1] for _ in batch]
    # labels = [_[2] for _ in batch]
    img, style_img, labels = [list(e) for e in zip(*batch)]
    # sample style images and labels
    style_img = list(map(lambda i: style_img[i], extract_idx))
    style_label = list(map(lambda i: labels[i], extract_idx))
    
    img.extend(style_img)
    labels.extend(style_label)
    
    # shuffle the batch:
    combine = list(zip(img, labels))
    random.shuffle(combine)
    img, labels = zip(*combine)
    out = None
    ...
    return torch.stack(img, 0, out=out), labels
