...
from ..datasets import DATASETS, dict_collate_fn, pair_collate_fn
...
def get_collate_fn(name, num_extra=0):
    if name is not None and name == 'dict_collate_fn':
        return dict_collate_fn
    elif name is not None and name == 'pair_collate_fn':
        return lambda batch: pair_collate_fn(batch, num_extra)
    else:
        return None
...
class BaseRunner(ABC):
    ...
    def load_checkpoint(self, ckpt_filename):
        ...
        if ckpt_filename is not None:
            load_checkpoint(net=self.model, 
                            lr_scheduler=getattr(self, 'lr_scheduler', None), 
                            optimizer=getattr(self, 'optimizer', None), 
                            filename=ckpt_filename, cfg=self._cfg)
...
class BaseTrainer(BaseRunner):
    def __init__(self, cfg, map_dataset_statics=None):
        ...
        self.collate_fn = get_collate_fn(self._cfg['collate_fn'], self._cfg.get('extra_samples', 0))
        transforms = TRANSFORMS.from_dict(cfg['train_augmentation'])
        self.dataset = DATASETS.from_dict(cfg['dataset'],
                                     transforms=transforms)
        self.get_dataset_statics(self.dataset, map_dataset_statics)
        self.train_sampler = get_sampler(self._cfg['distributed'], self.dataset)
        # --------------------------------------------------------------------------------------------
        ...
        self.validation_set = self.get_validation_dataset(cfg)
        self.validation_loader = None
        if self.validation_set is not None:
            val_bs = self._cfg.get('val_batch_size')
            if val_bs is None:
                val_bs = self._cfg['batch_size']
            self.validation_loader = torch.utils.data.DataLoader(dataset=self.validation_set,
                                                                 batch_size=val_bs,
                                                                 num_workers=self._cfg['workers'],
                                                                 shuffle=False,
                                                                 collate_fn=self.collate_fn)
