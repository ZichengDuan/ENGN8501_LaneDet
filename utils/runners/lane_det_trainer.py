...
class LaneDetTrainer(BaseTrainer):
    ...
    def run(self):
        ...
        self.model.train()
        num_samplers = len(self.dataset)
        print(f"There are {num_samplers} in this dataset")
        epoch = self._cfg.get('start_epoch', 0)
        ...
        while epoch < self._cfg['num_epochs']:
            ...
            if getattr(self.dataset, 'have_aug', False) and not self._cfg['distributed']:
                print("resampling some augmented data")
                origin_idcs = list(range(self.dataset.first_aug_idx))
                aug_dics = random.sample(list(range(self.dataset.first_aug_idx, num_samplers)), self.dataset.num_aug)
            ...
            if epoch in set([349, 374]):
                save_checkpoint(net=self.model.module if self._cfg['distributed'] else self.model,
                                optimizer=None,
                                lr_scheduler=None,
                                filename=os.path.join(self._cfg['exp_dir'], f'model_{epoch}.pt'))
            ...
            save_checkpoint(net=self.model.module if self._cfg['distributed'] else self.model,
                            ...
                            epoch=epoch,
                            filename=os.path.join(self._cfg['exp_dir'], 'latest.pt'))
