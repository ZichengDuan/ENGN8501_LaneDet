...
def load_checkpoint(net, optimizer, lr_scheduler, filename, strict=True, cfg=None):
    try:
    ...
    if lr_scheduler is not None and checkpoint['lr_scheduler'] is not None:
        try:  # Shouldn't be necessary, but just in case
            ...
            pass
    if cfg is not None and checkpoint.get('epoch', None) is not None:
        try:
            cfg['start_epoch'] = checkpoint['epoch']
        except RuntimeError:
            warnings.warn('Incorrect lr epoch state dict.')
            pass
