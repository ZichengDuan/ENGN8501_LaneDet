...
__all__ = [
    ...
    'Normalize_TwoImages',
    'RandomAffine',
    'RandomAffine_TwoImages',
    'RandomApply',
    'RandomCrop',
    'RandomHorizontalFlip',
    'RandomHorizontalFlip_TwoImages',
    'RandomLighting',
    'RandomResize',
    'RandomRotation',
    'RandomScale',
    ...
    'RandomErasing',
    'RandomMasking',
    'RandomMasking_TwoImages',
    'RandomMask_or_Erasing',
]
...
@TRANSFORMS.register()
class RandomMask_or_Erasing(object):
    '''
    -------------------------------------------------------------------------------------
    ...
    def __init__(self, probability=0.5, sl=0.02, sh=0.15, r1=0.2, mean=[0.4914, 0.4822, 0.4465]):
    ...
    def RandomErasing(self, img, target):
    ...
    def RandomMasking(self, img, target):
        h = img.size()[1]
        w = img.size()[2]
    ...
    def __call__(self, img, target):
        ...
        #     img, target = self.RandomErasing(img, target)
        # else:
        img, target = self.RandomMasking(img, target)
...
@TRANSFORMS.register()
class Compose_TwoImages(object):
    def __init__(self, transforms):
        self.transforms = [t if callable(t) else TRANSFORMS.from_dict(t) for t in transforms]
...
class Resize_TwoImages(object):
    ...
    def __call__(self, image1, image2, target=None):
        w_ori1, h_ori1 = F._get_image_size(image1)
        w_ori2, h_ori2 = F._get_image_size(image2)
        assert w_ori1 == w_ori2 and h_ori1 == h_ori2, "The size of two images should be same, get img1: ({w_ori1}, {h_ori1}), img2: ({w_ori2}, {h_ori2})"
        
        image1, target = self.parse_resize(image1, target, self.size_image, self.size_label, (h_ori1, w_ori1), self.ignore_x)
        image2, _ = self.parse_resize(image2, None, self.size_image, self.size_label, (h_ori2, w_ori2), self.ignore_x)
        return image1, image2, target
...
@TRANSFORMS.register()
class RandomHorizontalFlip_TwoImages(object):
    def __init__(self, flip_prob, ignore_x=-2):
        self.flip_prob = flip_prob
        self.ignore_x = ignore_x

    def __call__(self, image1, image2, target=None):
        assert target is not None, "Target 2 is None, please check the data loading"
        
        t = random.random()
        if t < self.flip_prob:
            # flip for both images and targets
            ...
            if target is None or isinstance(target, str):
                ...
                if 'keypoints' in target.keys():
                    target['keypoints'] = F_kp.hflip(target['keypoints'],
                                                     F._get_image_size(image1)[0] / 2, self.ignore_x)
                if 'segmentation_mask' in target.keys():
                    target['segmentation_mask'] = F.hflip(target['segmentation_mask'])
...
@TRANSFORMS.register()
class ToTensor_TwoImages(object):
    def __init__(self, keep_scale=False, reverse_channels=False):
        # keep_scale = True => Images or whatever are not divided by 255
        # reverse_channels = True => RGB images are changed to BGR (the default behavior of openCV & Caffe,
        #                                                           let's wish them all go to heaven,
    ...
    def __call__(self, image1, image2, target=None):
        image1 = self._pil_to_tensor(image1)
        image2 = self._pil_to_tensor(image2)
        target = self.label_to_tensor(target)
...
@TRANSFORMS.register()
class Normalize_TwoImages(object):
    def __init__(self, mean, std, normalize_target=True, ignore_x=-2):
        # Default values changed (2021.12.18)
        self.mean = mean
        self.std = std
    ...
    def __call__(self, image1, image2, target=None):
        ...
        if self.normalize_target and not (target is None or isinstance(target, str)):
            if isinstance(target, dict):
                if 'keypoints' in target.keys():
                    w1, h1 = F._get_image_size(image1)
                    w2, h2 = F._get_image_size(image2)
                    assert w1 == w2 and h1 == h2, "The size of two images should be same, get img1: ({w1}, {h1}), img2: ({w2}, {h2})"
                    target['keypoints'] = F_kp.normalize(target['keypoints'], h1, w1, self.ignore_x)
...
@TRANSFORMS.register()
class ColorJitter_TwoImages(object):
    """Randomly change the brightness, contrast, saturation and hue of an image.
    If the image is torch Tensor, it is expected
    to have [..., 3, H, W] shape, where ... means an arbitrary number of leading dimensions.
    If img is PIL Image, mode "1", "L", "I", "F" and modes with transparency (alpha channel) are not supported.
    ...
    def __call__(self, image1, image2, target):
        ...
        for fn_id in fn_idx:
            if fn_id == 0 and brightness_factor is not None:
                image1 = F.adjust_brightness(image1, brightness_factor)
                image2 = F.adjust_brightness(image2, brightness_factor)
            elif fn_id == 1 and contrast_factor is not None:
                image1 = F.adjust_contrast(image1, contrast_factor)
                image2 = F.adjust_contrast(image2, contrast_factor)
            elif fn_id == 2 and saturation_factor is not None:
                image1 = F.adjust_saturation(image1, saturation_factor)
                image2 = F.adjust_saturation(image2, saturation_factor)
            elif fn_id == 3 and hue_factor is not None:
                image1 = F.adjust_hue(image1, hue_factor)
                image2 = F.adjust_hue(image2, hue_factor)
...
@TRANSFORMS.register()
class RandomAffine_TwoImages(torch.nn.Module):
    ...
    def forward(self, image1, image2, target):
        
        assert image1 is not None, "Image1 2 is None, please check the data loading"
        assert image2 is not None, "Image 2 is None, please check the data loading"
        assert target is not None, "Target 2 is None, please check the data loading"
        
        img1_size = F._get_image_size(image1)
        img2_size = F._get_image_size(image2)
        assert img1_size[0] == img2_size[0] and img1_size[1] == img2_size[1], f"Size of two images should be same, get size img1: {img1_size}, size img2: {img2_size}"
        ret = self.get_params(self.degrees, self.translate, self.scale, self.shear)
        image1 = F.affine(image1, *ret, resample=Image.LINEAR, fillcolor=0)
        image2 = F.affine(image2, *ret, resample=Image.LINEAR, fillcolor=0)
        ...
        if target is None or isinstance(target, str):
            ...
            if 'keypoints' in target.keys():
                target['keypoints'] = F_kp.affine(target['keypoints'], *ret,
                                                  height=img1_size[1], width=img1_size[0], ignore_x=self.ignore_x)
            if 'segmentation_mask' in target.keys():
                target['segmentation_mask'] = F.affine(target['segmentation_mask'], *ret,
                                                       resample=Image.NEAREST, fillcolor=255)         
        else:
